<div>
    <header>
        <nav class="w-full py-3">
            <ul class="flex items-center justify-between w-full text-gray-600">
                <li>
                    <h2 class="ml-4 text-2xl font-semibold lg:ml-10 xl:text-3xl">Surat Lamaran Saya</h2>
                </li>
                <li>
    ...
    <div class="flex justify-center w-full mt-3 text-gray-600">
        <div class="w-9/12 p-8 bg-white rounded shadow-md h-1/4">
            ...
            <div>
                <h1 class="mb-4 text-2xl font-semibold text-center">Balasan Surat dari Perusahaan</h1>
                <div class="flex flex-col justify-between w-full h-full">
                    @if ($application->company_letter && $application->status == 1)
                        {!! $application->company_letter !!}
                    @elseif ($application->status == 0)
                        <h2>Maaf, lamaran anda ditolak.</h2>
                    @else
                        <h2>Mohon ditunggu lamaran telah diproses.</h2>
                    @endif
