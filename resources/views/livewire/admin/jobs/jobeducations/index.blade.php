<div>
    <header>
        <nav class="w-full pt-14 lg:py-3">
            <ul class="flex items-center justify-between w-full text-gray-600">
                <li>
                    <h2 class="ml-4 text-2xl font-semibold lg:ml-10 xl:text-3xl">Data Pendidikan</h2>
                </li>
    ...
    <div class="mx-4 lg:ml-8">
        ...
        <div>
            <a wire:navigate href="/admin/jobeducations/create">
                <button type="submit"
                    class="px-4 py-2 mb-6 text-sm font-semibold text-white bg-blue-600 rounded-lg hover:bg-blue-800">Buat
                    Data</button>
            </a>
        </div>
        ...
        @endif
        @if ($jobeducations->where('deleted_at', null)->isEmpty())
            <div class="mt-24 text-gray-600">
                <h1 class="mb-2 text-2xl font-semibold text-center lg:text-3xl">Data pendidikan tidak tersedia.</h1>
            </div>
        @else
            <table class="min-w-full overflow-hidden bg-white rounded-lg shadow-md">
                ...
                <tbody class="">
                    @foreach ($jobeducations as $jobeducation)
                        <div wire:key="{{ $jobeducation->id }}">
                            <tr class="hover:bg-gray-100">
                                <td class="px-4 py-3">{{ $loop->iteration }}</td>
                                <td class="px-4 py-3">{{ $jobeducation->name }}</td>
                                <td class="px-4 py-3">
