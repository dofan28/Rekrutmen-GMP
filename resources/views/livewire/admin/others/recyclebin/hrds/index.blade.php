    ...
    <header>
        <nav class="w-full pt-14 lg:py-3">
            <ul class="flex items-center justify-between w-full text-gray-600">
                <li>
                    <h2 class="ml-4 text-2xl font-semibold lg:ml-10 xl:text-3xl">Tempat Sampah | Data HRD</h2>
                </li>
    ...
    <div class="mx-4 lg:ml-8">
        ...
        @if ($hrds->isEmpty())
            <div class="mt-24 text-gray-600">
                <h1 class="mb-2 text-2xl font-semibold text-center lg:text-3xl">Data HRD di tempat sampah tidak
                    tersedia.
                </h1>
            </div>
        @else
                ...
                <tbody class="">
                    @foreach ($hrds as $hrd)
                        <tr class="hover:bg-gray-100">
                            ...
                            <td class="px-4 py-3">{{ $hrd->hrd_position }}</td>
                            <td class="px-4 py-3"><a wire:navigate href="/admin/hrds/{{ $hrd->id }}"
                                    class="font-medium text-blue-600 hover:underline">Detail</a></td>
                            <td class="px-4 py-3">
                                <div class="flex flex-col justify-start gap-2 lg:flex-row">

                                    <a wire:navigate href="/admin/others/recyclebin/hrds/{{ $hrd->id }}/restore"
                                        class="font-medium text-blue-600 hover:underline">Pulihkan</a>
                                    <form action="/admin/others/recyclebin/hrds/{{ $hrd->id }}/force" method="post"
                                        class="inline-block">
                                        @csrf
                                        @method('delete')
