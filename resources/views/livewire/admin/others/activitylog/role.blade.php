<div>
    <header>
        <nav class="w-full pt-14 lg:py-3">
            <ul class="flex items-center justify-between w-full text-gray-600">
                <li>
                    <h2 class="ml-4 text-2xl font-semibold lg:ml-10 xl:text-3xl">Peran</h2>
                </li>
                <li>
    ...
    <div class="w-11/12 p-6 mx-auto mt-10 text-gray-600 bg-white rounded-md shadow-md lg:w-3/4">
        ...
        <div class="mb-4">
            <label class="text-gray-600">ID Peran</label>
            <p class="font-semibold">{{ $role->causer_id }}</p>
        </div>
        <div class="mb-4">
            <label class="text-gray-600">Peran: </label>
            <p class="font-semibold">
                @if ($role->causer_type == 'App\Models\Applicant')
                    Pelamar
                @elseif ($role->causer_type == 'App\Models\Hrd')
                    HRD
                @elseif ($role->causer_type == 'App\Models\Admin')
                    Admin
                @else
                    Null
                @endif
