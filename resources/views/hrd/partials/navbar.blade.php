{{-- <header>
    <nav class="w-full px-12 py-6 sm:px-4 lg:px-12">
        <ul class="flex items-center justify-between w-full h-full text-gray-600">
            <li>
                <h2 class="text-3xl font-semibold sm:text-lg lg:text-3xl">{{$title}}</h2>
            </li>
            <li>
                <div class="relative">
            ...
            <li>
                <div class="flex items-center justify-between w-56 h-full sm:w-28 lg:w-56">
                    <div>
                    ...
                    <div class="flex flex-row items-center px-3 py-1 shadow-sm justify-evenly rounded-2xl bg-gray-50">
                        <div class="flex flex-col items-end h-full mr-2">
                            <h6 class="text-sm font-semibold sm:text-xs lg:text-sm">
                            {{Auth::guard('hrd')->user()->full_name ?? ''}}</h6>
                            <span class="text-xs">HRD</span>
                        </div>
                        @if (Auth::guard('hrd')->user()->photo ?? '')
                        <img class="rounded-full"
                            src="{{ asset('storage/' . Auth::guard('hrd')->user()->photo) }}"
                            width="35px" srcset="">
                    @else
                        <img class="rounded-full" src="/storage/images/hrd/default.jpg" width="35px"
                            srcset="">
                    @endif
                    </div>
                </div>
