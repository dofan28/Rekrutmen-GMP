...
namespace App\Livewire\Admin\Others\Recyclebin\Applications;
...
class Index extends Component
{
    ...
    public function render()
    {
        $applications = Application::query()
            ->when($this->search, function ($query) {
                $query->where(function ($subQuery) {
                    $subQuery->where('applicant_letter', 'like', '%' . $this->search . '%')
                        ->orWhere('company_letter', 'like', '%' . $this->search . '%')
                        ->orWhereHas('applicant', function ($applicantQuery) {
                            $applicantQuery->where('email', 'like', '%' . $this->search . '%')
                                ->orWhereHas('applicantdata', function ($applicantdataQuery) {
                                    $applicantdataQuery->where(function ($applicantDataSubQuery) {
                                        $applicantDataSubQuery->where('ktp_number', 'like', '%' . $this->search . '%')
                                ...
                                ->orWhereHas('contact', function ($contactQuery) {
                                    $contactQuery->where(function ($contactSubQuery) {
                                        $contactSubQuery->where('street', 'like', '%' . $this->search . '%')
                                ...
                                ->orWhereHas('educationalbackground', function ($educationalbackgroundQuery) {
                                    $educationalbackgroundQuery->where(function ($eduBackgroundSubQuery) {
                                        $eduBackgroundSubQuery->where('institution', 'like', '%' . $this->search . '%')
                                ...
                                ->orWhereHas('workexperience', function ($workexperienceQuery) {
                                    $workexperienceQuery->where(function ($workExpSubQuery) {
                                        $workExpSubQuery->where('company', 'like', '%' . $this->search . '%')
                                ...
                                ->orWhereHas('organizationalexperience', function ($organizationalexperienceQuery) {
                                    $organizationalexperienceQuery->where(function ($orgExpSubQuery) {
                                        $orgExpSubQuery->where('organizational_name', 'like', '%' . $this->search . '%')
                                            ->orWhere('position', 'like', '%' . $this->search . '%')
        ...
        return view('livewire.admin.others.recyclebin.applications.index', [
